	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		...
		if(!empty($cycleCount->wh) && !empty($cycleCount->Custid) && !empty($cycleCount->Batchid)){
			try{
				...
				$cycleCountData = $cycleCountDataObj->cycleCountIdGetBin($cycleCount->wh, $cycleCount->Custid, $cycleCount->Batchid);
                //echo json_encode($cycleCountData); exit;
				...
				echo json_encode(array(
					"status" => 1,
					"rackLocationData" => $cycleCountDataArray
				));
			}catch(Exception $ex){
