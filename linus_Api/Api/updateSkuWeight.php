<?php
	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		...
		if(!empty($skuWeightDateInputs->sku_Name) && !empty($skuWeightDateInputs->weight)){
			try{
				$decoded_data = JWT::decode($token, $secret_key, array('HS512'));

				$skuData = $skuDataObj->updateSkuWeight($skuWeightDateInputs->sku_Name, $skuWeightDateInputs->weight);
