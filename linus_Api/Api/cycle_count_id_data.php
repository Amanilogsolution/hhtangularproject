	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		...
		if(!empty($cycleCount->wh) && !empty($cycleCount->Custid) && !empty($cycleCount->Batchid)){
			try{
				...
				$cycleCountData = $cycleCountDataObj->cycleCountIdData($cycleCount->wh, $cycleCount->Custid, $cycleCount->Batchid);
				...
				while($row = sqlsrv_fetch_array($cycleCountData, SQLSRV_FETCH_ASSOC))
				{
					$cycleCountDetails =  array(
                        "TotalAllotBIN" =>$row['TotalAllotBIN'],
                        "OpenForCount" =>$row['OpenForCount'],
				...
				echo json_encode(array(
					"status" => 1,
					"cycleCountData" => $cycleCountDataArray
				));
			}catch(Exception $ex){
