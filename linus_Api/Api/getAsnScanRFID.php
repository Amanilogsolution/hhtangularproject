	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		...
		if(!empty($asnScanDataInputs->wh) && !empty($asnScanDataInputs->BoxNo)){
			try{
				...
				//$asnScanQtyData = $asnScanDataObj->getAsnScanQtyData($asnScanDataInputs->wh, $asnScanDataInputs->BoxNo);
				...
				$asnScanQtyDataArray = array();
				$asnScanQtyDataArray["records"]=array();
				while($row = sqlsrv_fetch_array($asnScanQtyData, SQLSRV_FETCH_ASSOC))
				{
					$scanBoxsDetails =  array(
                        "Scanqty" 			    => $row['Scanqty']
					);
					array_push($asnScanQtyDataArray["records"], $scanBoxsDetails);
				}
				...
				$asnScanTableData = $asnScanDataObj->AsnScanRFIDTableData($asnScanDataInputs->wh, $asnScanDataInputs->BoxNo,  $records_per_page, $numer_of_records);
				...
				echo json_encode(array(
					...
					"asnScanQtyData"   		=> $asnScanQtyDataArray,
