<?php
	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		...
		$token = explode(" ", $jwt)[1];
		$scanBin = json_decode(file_get_contents("php://input"));
		if(!empty($scanBin->WH) && !empty($scanBin->Custid) && !empty($scanBin->Location)){
			try{
				$decoded_data = JWT::decode($token, $secret_key, array('HS512'));

				$BinData = $BinDataObj->ScanTransferToBin($scanBin->WH, $scanBin->Custid, $scanBin->Location);
