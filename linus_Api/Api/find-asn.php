<?php
	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		...
		$token = explode(" ", $jwt)[1];
		$findAsnInputs = json_decode(file_get_contents("php://input"));
		if(!empty($findAsnInputs->locationId) && !empty($findAsnInputs->asnno))
		{
			$headers = getallheaders();
			$jwt = $headers["authorization"];
			$token = explode(" ", $jwt)[1];
			try{
				...
				$numer_of_records =3;
				$findAsnData = $findAsnObj->findAsn($findAsnInputs->locationId,$findAsnInputs->asnno,$records_per_page, $numer_of_records);
				...
				$findAsnDataArray["numberOfRecords"]=$findAsnObj->findAsnListCount($findAsnInputs->locationId,$findAsnInputs->asnno);
