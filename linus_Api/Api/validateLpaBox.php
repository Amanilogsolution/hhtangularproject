	...
	if($_SERVER['REQUEST_METHOD'] === "POST")
	{
		...
		if(!empty($lpaInputs->wh) && !empty($lpaInputs->WhLocation))
		{
			try{
				...
				$validLpaData = $lpaObj->ValidateLpaBOX($lpaInputs->wh, $lpaInputs->WhLocation);
                $custId = $validLpaData['records'][0]['custId']; //exit;
                $custInv = $validLpaData['records'][0]['custInv'];
                $gridDataValidateLpaBOX = $lpaObj->gridDataValidateLpaBOX($lpaInputs->wh, $lpaInputs->WhLocation, $custId, $custInv);
				...
				echo json_encode(array(
                    ...
                    "gridData" => $gridDataValidateLpaBOX
