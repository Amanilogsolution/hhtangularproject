<?php
	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		...
		if(!empty($binCount->wh) && !empty($binCount->Custid) && !empty($binCount->tobin) && !empty($binCount->fromBin)){
			try{
				$decoded_data = JWT::decode($token, $secret_key, array('HS512'));

				$BinData = $BinDataObj->saveBinToBin($binCount->wh, $binCount->Custid, $binCount->tobin, $binCount->fromBin, $binCount->body, $decoded_data->data->name);
